...
static int ov772x_write_array(struct i2c_client *client, const ov_camera *vals)
{
    ...
    while ((vals->regAddr != 0xff) || (vals->regVal != 0xff)) {
        ...
        dev_vdbg(&client->dev, "array: 0x%02x, 0x%02x", vals->regAddr, vals->regVal);
        if (ret < 0) {
			     ...
			     const struct ov772x_win_size *win)
{
...
#if 0
    ret = ov772x_write_array(client, ov7725_15fps_pclk12Mhz);
    if (ret < 0)
    ...
    ov772x_write(client, 0x2a, 0x00);
    ov772x_write(client, 0x2b, 0x00);
    ov772x_write(client, 0x33, 0x00);
    ov772x_write(client, 0x34, 0x00);
    ov772x_write(client, 0x2d, 0x00);
    ov772x_write(client, 0x2e, 0x00);
    ov772x_write(client, 0x0e, 0x65);
#endif
			...
			const struct i2c_device_id *did)
{
	...
	JALY_TRACE(DEBUG_LEVEL);
	printk("%s: client->name = %s client->addr = 0x%x\n", __FUNCTION__, client->name, client->addr);
